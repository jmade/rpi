...
def colorize_frame(frame):
	def colorize(frame,strip):
		for i in range(TOTAL_PIXEL_COUNT):
			...
			blue_pixel = int(pixel[1])
			green_pixel = int(pixel[2])
			color = Color(red_pixel,blue_pixel,green_pixel)
...
def read_frame(command,strip,frame_count=1):
	...
	if frame_count == 0:
		print("Frame Count is Zero, Running Loop Indefinitly...")
		while True:
		...
		print("Running Loop for: ",frame_count)
